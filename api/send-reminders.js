...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const appt of appointments) {
      ...
      const practitionerName = appt.practitioners

        ? appt.practitioners.name

        : 'your practitioner';

      const locationName = appt.locations
      ...
      if (!sendResp.ok) {
        ...
        const sendErrorBody = await sendResp
      ...
      if (!markResp.ok) {
        ...
        errors.push({
          ...
          error: 'Sent but could not mark as reminded'
